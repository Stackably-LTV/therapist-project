...
import { createQuestion } from '@/components/bd2b831f136e';
export async function POST(request, { params }) {
    try {
        ...
        const result = await createQuestion(courseId, lessonId, assessmentId, auth.userId, body);
        if (!result.ok)
            return NextResponse.json({ error: result.error }, { status: result.status });
        return NextResponse.json(result.data, { status: 201 });
    }
    catch (err) {
        console.error('[api/therapist/courses/.../questions] POST error', err);
